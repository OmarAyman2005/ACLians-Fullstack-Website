...
export function EventDetailsModal({ event, onClose }) {
    ...
    if (event.eventType === "conference") {
        ...
        if (event.conferenceWebsiteLink) extraFields.push({ label: "Website", value: <a href={event.conferenceWebsiteLink} target="_blank" rel="noopener noreferrer" className="text-root-primary underline">{event.conferenceWebsiteLink}</a> });
                                    ...
                                    <ul className="space-y-2">
                                        {participants.map((p) => {
                                            ...
                                            const booth =
                                                isString
                                                    ? null
                                                    : p?.booth || p?.boothNumber || p?.stallNumber || p?.vendor?.booth || null;
                                            const contact =
                                                isString
                                                    ? null
                                                    : p?.email || p?.contact || p?.vendor?.email || p?.vendor?.contact || null;
                                            const key = isString ? `vendor-${name}` : p._id || p.id || `vendor-${name}-${booth || "n"}`;
                                            return (
