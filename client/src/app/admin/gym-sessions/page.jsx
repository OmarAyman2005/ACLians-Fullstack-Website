...
export default function AdminGymSessionsPage() {
  ...
  const { minISO, maxISO, days } = useMemo(() => buildRollingDays28(today), [today]);

  const coaches = useMemo(() => (type ? COACHES_BY_TYPE[type] || [] : []), [type]);
  ...
  function handleCapacityChange(e) {
    ...
    else if (n < 1 || n > 30) setCapacityErr("Capacity must be between 1 and 30.");
    else setCapacityErr("");
  }
  ...
  function handleCalendarPick(iso) {
    if (!step3Coach) {
      showOrderMsg("Please choose Category, Type, and Coach before picking a date.");
      return;
    }
    setMsg({ tone: "", text: "" });
    setDateISO(iso);
  ...
  async function submit(e) {
    ...
    if (!step2Type) return showOrderMsg("Please select a Type after choosing a Category.");
    if (!step3Coach) return showOrderMsg("Please select a Coach after choosing a Type.");
    if (!step4Date) return showOrderMsg("Please pick a Date after selecting a Coach.");
    if (isFriSat)
      return showOrderMsg(
        "Fridays and Saturdays are non-schedulable (policy). Choose Sunday–Thursday."
      );
    if (!step5Timing) return showOrderMsg("Please pick a Timing slot after selecting a Date.");
    if (!validCapacity(capacity)) {
      return showOrderMsg(capacityErr || "Capacity must be an integer between 1 and 30.");
    }
    if (bookedSet.has(`${startTime}-${endTime}`)) {
      return showOrderMsg("This timing is already created for the selected type and date.");
    }

    setSubmitting(true);
    try {
      const res = await fetch(gymSessionsUrl, {
        method: "POST",
      ...
      setBookedSet((prev) => {
        ...
        p.add(`${data?.data?.startTime ?? startTime}-${data?.data?.endTime ?? endTime}`);
        return p;
      });
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] py-8">
      ...
      <div className="flex items-center justify-end mb-4 px-4">
        <Link
          href="/admin"
          className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-primary text-root-primary hover:bg-primary-hover transition"
        >
      ...
      {showSuccess && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="relative z-10 rounded-2xl border border-emerald-500/40 bg-neutral-900 text-emerald-100 text-center px-8 py-6 shadow-xl">
            ...
            <div className="text-sm opacity-90">Your gym session has been saved successfully.</div>
          </div>
        </div>
      )}
      ...
      <section className="mx-auto max-w-4xl rounded-2xl bg-surface text-root-primary border border-root shadow-elevated p-6 md:p-8">
        <h1 className="text-center text-2xl md:text-3xl font-semibold mb-6">Create Gym Session</h1>
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block mb-1 text-sm text-root-secondary">Category</label>
              <select
            ...
            <div>
              <label className="block mb-1 text-sm text-root-secondary">Type</label>
              <Gate
              ...
              >
                <select
                  className={`input-surface w-full ${!step1Category ? "opacity-60 cursor-not-allowed" : ""}`}
                  value={type}
                  onChange={(e) => {
          ...
          {/* Coach */}
          <div>
            <label className="block mb-1 text-sm text-root-secondary">Coach</label>
            <Gate
              blocked={!step2Type}
            ...
            >
              <select
                className={`input-surface w-full ${!step2Type ? "opacity-60 cursor-not-allowed" : ""}`}
                value={coach}
                onChange={(e) => {
          ...
          {!step3Coach ? (
            <div className="rounded-xl border border-root bg-surface/60 p-4 text-root-primary">
              Please select a gym session (<b>category</b>, <b>type</b>, and <b>coach</b>) to view
              availability.
            </div>
          ) : !dateISO ? (
            <div className="rounded-xl border border-root bg-surface/60 p-4 text-root-primary">
            ...
            <div className="rounded-xl ring-1 ring-yellow-500/30 bg-yellow-500/10 p-4 text-yellow-100">
              ⚠️ Fridays and Saturdays are <b>non-schedulable</b> (policy). Please choose a day from{" "}
              <b>Sunday to Thursday</b>.
            </div>
          ) : (
            <div>
              <label className="block mb-2 text-sm text-primary">Pick a fixed timing</label>
          ...
          {/* Capacity */}
          <div>
            <label className="block mb-1 text-sm text-root-secondary">Max Participants</label>
            <Gate
              blocked={!step5Timing}
            ...
            >
              <input
                className={`input-surface w-full ${!step5Timing ? "opacity-60 cursor-not-allowed" : ""}`}
                type="number"
                min={1}
                max={30}
                step={1}
            ...
            {capacityErr && <p className="text-sm mt-1 text-red-300">{capacityErr}</p>}
          </div>

          <button type="submit" className="btn btn-primary w-full disabled:opacity-60" disabled={!canCreate}>
...
/* ---------------- MiniCalendar28 ---------------- */
function MiniCalendar28({ value, days, minISO, maxISO, disabled, onTryBeforeEnabled, onChange }) {
  const label = useMemo(() => labelForRange(days), [days]);

  return (
    <section>
      <div className="mb-2 text-sm text-primary text-left">{label}</div>
      <div className={`rounded-xl ring-1 ring-neutral-700 bg-highlight p-3 ${disabled ? "opacity-60" : ""}`}>
          ...
          {days.map((d) => {
            ...
            return (
              <button
                ...
                type="button"
                disabled={isDisabled}
                onClick={() => (!isDisabled ? onChange(iso) : onTryBeforeEnabled?.())}
                className={[
                ...
                title={isOff ? "Friday/Saturday (non-schedulable)" : d.date.toDateString()}
              >
...
function labelForRange(days) {
  ...
  if (sameMonth) return first.toLocaleDateString(undefined, { month: "long", year: "numeric" });
